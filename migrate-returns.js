...
function insertSampleReturns() {
    ...
    db.all('SELECT id, product_id, quantity_sold, sale_price, total_amount FROM sales LIMIT 5', (err, sales) => {
        ...
        db.all('SELECT id, name FROM products LIMIT 5', (err, products) => {
            ...
            // Get return reasons
            db.all('SELECT id, reason_code, reason_name FROM return_reasons', (err, reasons) => {
                if (err) {
                    console.error('Error fetching return reasons:', err.message);
                    return;
                }
                ...
                sampleReturns.forEach(returnItem => {
                    ...
                    `, [
                        ...
                        if (err) {
                            ...
                            // Insert initial activity for this return
                            db.run(`
                                INSERT INTO return_activities (return_id, activity_type, activity_description, performed_by) 
                                VALUES (?, ?, ?, ?)
                            `, [this.lastID, 'created', `Return request created by ${returnItem.customer_name}`, returnItem.processed_by], (activityErr) => {
                                if (activityErr) {
                                    console.error('Error inserting return activity:', activityErr.message);
                                }
                            });
                        }
                    });
                ...
                setTimeout(() => {
                    insertReturnActivities(sampleReturns);
                }, 2000);
            });
        });
    });
...
function insertReturnActivities(sampleReturns) {
